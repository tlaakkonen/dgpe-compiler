...
impl BlockMatrix {
    ...
    pub fn block_row_add(&mut self, row1: usize, row2: usize, mat: &Matrix, rec: &mut (impl NonlocalRecorder + ?Sized)) {
        ...
        for r in 0..decomp.c.num_cols() {
            ...
            for i in 0..mat.shape.0 {
                for j in 0..mat.shape.1 {
                    if decomp.c[(i, r)] == GF2::ONE && decomp.f[(r, j)] == GF2::ONE {
                        self.row_add(self.indices[row1] + j, self.indices[row2] + i);
                    }
                }
            }
        }
    ...
    pub fn eliminate_col(&mut self, arch: &GlobalArch, row: usize, col: usize, rec: &mut (impl NonlocalRecorder + ?Sized)) {
    ...
    pub fn eliminate_row(&mut self, arch: &GlobalArch, row: usize, col: usize, rec: &mut (impl NonlocalRecorder + ?Sized)) {
        let leaves = (0..self.parts)
    ...
    pub fn rowcol(&mut self, arch: &GlobalArch, rec: &mut (impl NonlocalRecorder + ?Sized)) {
        ...
        for _ in 0..self.parts {
            ...
            let row = (0..self.parts)
                ...
                .unwrap();
            self.eliminate_col(&arch, row, row, rec);
            self.eliminate_row(&arch, row, row, rec);
    ...
    pub fn permrowcol(&mut self, arch: &GlobalArch, rec: &mut (impl NonlocalRecorder + ?Sized)) {
        let mut arch = arch.clone();
        let mut visited = HashSet::new();
        for _ in 0..self.parts {
            ...
            self.eliminate_col(&arch, row, col, rec);
            self.eliminate_row(&arch, row, col, rec);
